...
const POST = router.post('/', (req, res) => {
    ...
    con.connect(error => {
        ...
        if (!body.internship_period.includes("/")) return returnError(res, ["missing /"]);
        if (!body.internship_period.includes("-")) return returnError(res, ["missing -"]);
        ...
        con.query(
            ...
            function(error, results, fields){
                ...
                fs.mkdirSync(path.join(userHomeDir, body.email_dir , body.internship_period.replace("-","to").split("/").join("-")), { recursive: true })
                fs.mkdirSync(path.join(userHomeDir, body.resume_dir , body.internship_period.replace("-","to").split("/").join("-")), { recursive: true })
